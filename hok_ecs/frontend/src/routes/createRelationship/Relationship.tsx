...
import { EntityOrComponentValue, RelationshipType } from '../CreateRelationship';
...
const Relationship: React.FC<Props> = ({ sourceValue, destinationValue, relationshipType }) => {
  return (
    <div className="bg-gradient text-center d-flex border border-2 rounded border-primary m-4 p-4 w-100 justify-content-center align-items-center">
      <div className="w-100">
        <RelationshipMemberBadge
          value={
            sourceValue ? `${sourceValue.type.toUpperCase()}: ${sourceValue.label}` : undefined
          }
        ...
        <RelationshipMemberBadge
          value={
            destinationValue
              ? `${destinationValue.type.toUpperCase()}: ${destinationValue.label}`
              : undefined
          }
